...
(async () => {
  ...
  /*
    Preklic sej gre PRVI. Obremenitveni del spodaj porabi proračun na IP
    (30 zahtev / 15 min), zato prijava za ta test po njem ne bi šla več skozi.
  */
  console.log('1) odjava prekliče žeton');
  if (!EMAIL || !GESLO) {
    ...
    if (p.koda !== 200 || !p.telo.token) {
      console.log('    — preskočeno: prijava ni uspela (' + p.koda + ')'
        + (p.koda === 429 ? ' — počakaj 15 min ali znova zaženi strežnik' : ''));
      preskoceno++;
    } else {
      ...
      if (po.koda === 200) {
        ...
        console.log('       Verjetno manjkata stolpca — poženi migracije/002-preklic-sej.sql');
      } else {
        trdi('žeton po odjavi ne deluje več', po.koda, 401);
      }

      // Nova prijava mora spet delati (preklic ne sme zakleniti računa).
      const znova = await zahteva('/api/auth/master-login', { email: EMAIL, password: GESLO });
      trdi('nova prijava po odjavi deluje', znova.koda, 200);
    }
  }

  // ── omejevanje poskusov ───────────────────────────────────────────────────
  console.log('\n2) prijava z e-pošto: 10 poskusov na 15 min po identiteti');
  ...
  console.log('\n3) omejitev je vezana na račun, ne na vse hkrati');
  ...
  console.log('\n4) ponarejen x-forwarded-for ne obide omejitve');
  ...
  console.log('\n5) OTP: 5 zahtev na 15 min po telefonu');
  ...
  if (napak === 0) console.log('Opomba: ta zagon je porabil proračun na IP — drugi zagon v 15 min bo javil lažne napake.');
